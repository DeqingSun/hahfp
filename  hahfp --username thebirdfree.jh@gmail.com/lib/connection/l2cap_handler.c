...
/*--------------------------------------------------------
 Default Configuration Table (conftab) and index constants
----------------------------------------------------------*/
static const uint16 default_conftab[] = 
    {
/* 00 */        L2CAP_AUTOPT_SEPARATOR,
                /* Flow & Error Control Mode */
/* 01 */        L2CA_AUTOPT_FLOW_MODE, 
                    /* Params - basic mode, no fallback mode */
/* 02 */            BKV_16_FLOW_MODE(L2CA_FLOW_MODE_BASIC,0),
                /* Local MTU - Exact value */
/* 03 */        L2CA_AUTOPT_MTU_IN,
                    /* param - MTU maximum for Stream - 895  */
/* 04 */            DEFAULT_L2CAP_LOCAL_MTU_MAXIMUM,
                /* Remote MTU - Minimum MTU Allowed */
/* 05 */        L2CA_AUTOPT_MTU_OUT,
                    /* param - Minimum MTU */
/* 06 */            L2CA_MTU_MINIMUM,
                /* Local Flush Timeout - default timeout = Infinite */
/* 07 */        L2CA_AUTOPT_FLUSH_IN,
                    /* Range of uint32 - Most Preferred  = DEFAULT = Infinite, 
                                         Least Preferred = DEFAULT = 0 */
/* 08 - 11 */       BKV_UINT32R(DEFAULT_L2CAP_FLUSH_TIMEOUT,0), 
                /* Remote Flush Timeout  - Accept Non-default Timeout*/
/* 12 */        L2CA_AUTOPT_FLUSH_OUT,
                    /* Range of uint32 - Most Preferred  = DEFAULT = Infinite, 
                                         Least Preferred = 0 */
/* 13 - 16 */       BKV_UINT32R(DEFAULT_L2CAP_FLUSH_TIMEOUT,0), 
                /* Ward off reconfiguration attempts */
/* 17 */        L2CA_AUTOPT_DISABLE_RECONF,
/* 18 */            L2CA_RECONF_DISABLE_ALL,
                /* Terminate this conftab */
/* 19 */        0xFF00             /* TO DO - replace with define, B-72017 */

    };

/* TO DO - LEAST and MOST preferred may be swapped in final release */
#define INDEX_FLOW_MODE                     2
#define INDEX_MTU_LOCAL                     4
#define INDEX_MTU_REMOTE                    6
#define INDEX_LOCAL_FLUSH_TO_LEAST_PREF     8
#define INDEX_LOCAL_FLUSH_TO_MOST_PREF      10
#define INDEX_REMOTE_FLUSH_TO_LEAST_PREF    13
...
void connectionHandleL2capConnectCfm(const L2CA_AUTO_CONNECT_CFM_T *cfm)
{
    ...
    {
        /* do nothing and wait for the success result */
        ;
    }
    else 
    {
    ...
    free(VmGetPointerFromHandle(cfm->config.flowspec));
    free(VmGetPointerFromHandle(cfm->config.qos));
}
...
static void sendL2capMapFixedCidReq(connectionL2capState *state, const CL_INTERNAL_L2CAP_MAP_CONNECTIONLESS_REQ_T* req)
{
    ...
    connectionConvertBdaddr_t(&bd_addr, &req->bd_addr);
...
void connectionHandleL2capMapFixedCidInd(const L2CA_MAP_FIXED_CID_IND_T* ind)
{
    ...
    connectionConvertBdaddr(&message->addr, &ind->bd_addr);
