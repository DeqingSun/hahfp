...
rfcomm_disconnect_status connectionConvertRfcommDisconnectStatus(uint16 status)
{
    switch (status)
    {
       	...
       	CASE(RFC_NORMAL_DISCONNECT, rfcomm_disconnect_normal_disconnect);
       	CASE(RFC_ABNORMAL_DISCONNECT, rfcomm_disconnect_abnormal_disconnect);
       	CASE(RFC_REMOTE_REFUSAL, rfcomm_disconnect_remote_refusal);
       	CASE(RFC_CHANNEL_ALREADY_EXISTS, rfcomm_disconnect_dlc_already_exists);
       	CASE(RFC_CONNECTION_REJ_SECURITY, rfcomm_disconnect_dlc_rej_security);
       	CASE(RFC_INVALID_CHANNEL, rfcomm_disconnect_invalid_server_channel);
       	CASE(RFC_UNKNOWN_PRIMITIVE, rfcomm_disconnect_unknown_primitive);
       	CASE(RFC_INVALID_PAYLOAD, rfcomm_disconnect_max_payload_exceeded);
       	CASE(RFC_INCONSISTENT_PARAMETERS, rfcomm_disconnect_inconsistent_parameters);
       	CASE(RFC_PEER_VIOLATED_FLOW_CONTROL, rfcomm_disconnect_credit_flow_control_protocol_violation);
        CASE(L2CA_DISCONNECT_LINK_LOSS, rfcomm_disconnect_l2cap_link_loss);
...
sds_status connectionConvertSdsResult(uint16_t result)
{
    switch(result)
    {
	CASE(SDS_SUCCESS, sds_status_success);
	CASE(SDS_PENDING, sds_status_pending);
	CASE(SDS_ERROR_UNSPECIFIED, sds_status_unspecified_error);
    CASE(SDS_ERROR_INVALID_SR_DATA, sds_status_invalid_sr_data_error);
    CASE(SDS_ERROR_INVALID_SR_HANDLE, sds_status_invalid_sr_handle_error);
    CASE(SDS_ERROR_INSUFFICIENT_RESOURCES, sds_status_insufficient_resources);
...
l2cap_map_connectionless_status connectionConvertL2capMapFixedCidResult(uint16 result)
{
    switch(result)
    {
	CASE(L2CA_MISC_SUCCESS, l2cap_map_connectionless_success);
	CASE(L2CA_MISC_INITIATING, l2cap_map_connectionless_initiating);
	CASE(L2CA_MISC_OUT_OF_MEM, l2cap_map_connectionless_out_of_memory);
    CASE(L2CA_MISC_INVALID_PSM, l2cap_map_connectionless_invalid_psm);
    CASE(L2CA_MISC_INVALID_CID, l2cap_map_connectionless_invalid_cid);
    CASE(L2CA_MISC_FAILED, l2cap_map_connectionless_failed);
    CASE(L2CA_MISC_TIMEOUT, l2cap_map_connectionless_timed_out);
    CASE(L2CA_MISC_ALREADY_REGISTERED, l2cap_map_connectionless_already_registered);
    coerce: return (sds_status)result;       
	default:
	    CL_DEBUG_INFO(("Unrecognised sds_status %d \n",result));
