...
*/
static void handleInternalSetSecurityModeCfm(connectionSmState* smState, const DM_SM_INIT_CFM_T* cfm)
{
...
{
	bdaddr bd_addr;
    connectionConvertBdaddr(&bd_addr, &ind->conn_setup.connection.bd_addr);
	
	/* If this is not an indication for SDP and the address matches one we're searching... */
    if(!(ind->conn_setup.connection.service.protocol_id== SEC_PROTOCOL_L2CAP && 
...
*/
void connectionHandleSmLinkKeyReqIndOld(const DM_SM_LINK_KEY_REQUEST_IND_T* ind)
{
	bdaddr	addr;
	
	/* Convert from CSR to CCL format */
	connectionConvertBdaddr(&addr, &ind->bd_addr);
    connectionAuthSendLinkKeyOld(&addr);
}
...
*/
void connectionHandleSmLinkKeyReqInd(Task theAppTask, connectionSmState* smState, const DM_SM_LINK_KEY_REQUEST_IND_T* ind)
{
    bdaddr	addr;
    ...
    if(!connectionAuthSendLinkKey(&addr, ind->authenticated))
    {			
...
*/
void connectionHandleSmLinkKeyInd(Task theAppTask, connectionSmState* smState, const DM_SM_LINK_KEY_IND_T* ind)
{
    bool bonded = FALSE;
    ...
    {
        bonded = connectionAuthAddDevice(&addr, key_type, ind->key, FALSE, TRUE);
    }
    /* If non bonding (and not debug key) register key */
    else if(!auth_bits_set && key_type !=cl_sm_link_key_debug)
    {
        bonded = FALSE;
        (void) connectionAuthAddDevice(&addr, key_type, ind->key, FALSE, FALSE);
    }
...
DESCRIPTION
    ...
    from the Bluestack Security Manager.  This indicates that a pairing
    operation has been completed.  For a successful pairing complete, connection
    Library would have received DM_SM_LINK_KEY_IND before this message and completed the 
    Authentication procedure. Handle this message only in failure case since BlueStack
    will not send a DM_SM_LINK_KEY_IND.

RETURNS

*/
void connectionHandleSmBondingCfm(Task theAppTask, connectionSmState* smState, const DM_SM_BONDING_CFM_T* cfm)
{
     bdaddr    addr; 

     connectionConvertBdaddr(&addr, &cfm->bd_addr);

    /* Check to determine if Authentication is active. This message is expected only on ssp_mode4 */
    if(smState->authReqLock && smState->security_mode == sec_mode4_ssp)
    {
        authentication_status status = connectionConvertAuthStatus(cfm->status);
        /* Success cases, Autentication ends with legacy devices on receiving the DM_SM_LINK_KEY_IND. End the Authentication only for failure*/
        if(status != auth_status_success)
        {
...
void connectionHandleSmAuthoriseInd(Task theAppTask, const DM_SM_AUTHORISE_IND_T* ind)
{
    ...
    connectionConvertBdaddr(&message->bd_addr, &ind->cs.connection.bd_addr);
    message->protocol_id = connectionConvertProtocolId_t(ind->cs.connection.service.protocol_id);
...
/*****************************************************************************/
void connectionHandleDmAclOpenInd(Task task, connectionSmState* smState, const DM_ACL_OPENED_IND_T *ind)
{
    sendAclOpenedIndToClient(task, (BD_ADDR_T *)&ind->bd_addr, (ind->flags & DM_ACL_FLAG_INCOMING), ind->dev_class, connectionConvertHciStatus(ind->status));
	if(smState->authReqLock && smState->security_mode == sec_mode4_ssp)
    {
...
void handleSetSspSecurityLevelReq(connectionSmState* smState, connectionReadInfoState* infoState, const CL_INTERNAL_SM_SET_SSP_SECURITY_LEVEL_REQ_T* req)
{
	if(req->ssp_sec_level < ssp_secl_level_unknown)
	{
		if(req->protocol_id == protocol_unknown)
		{
            ...
            prim->security_level_default = connectionConvertSspSecurityLevel_t(req->ssp_sec_level, TRUE, req->authorised, req->disable_legacy);
            prim->config = 0;
            prim->mode3_enc = 0;
            prim->write_auth_enable = 0;
			VmSendDmPrim(prim);
...
void handleSecModeConfigReq(connectionSmState* smState, connectionReadInfoState* infoState, const CL_INTERNAL_SM_SEC_MODE_CONFIG_REQ_T* req)
{
	...
	if(infoState->version != bluetooth_unknown)
	{
		if(!smState->setSecurityModeLock)
		{
            ...
            /* Devs need access DM_SM_SEC_MODE_CONFIG_SEND_ACCESS_IND inds to stop sdp ping */
            prim->config = 0;
			if(req->debug_keys)
...
*/
void connectionHandleSetSecurityModeCfm(connectionSmState* smState, const DM_SM_INIT_CFM_T* cfm)
{
...
*/
void connectionHandleConfigureSecurityCfm(connectionSmState* smState, const DM_SM_INIT_CFM_T* cfm)
{
	if(smState->setSecurityModeLock)
	{
		if(smState->setSecurityModeLock != connectionGetCmTask())
...
void handleRegisterOutgoingReq(const CL_INTERNAL_SM_REGISTER_OUTGOING_REQ_T* req)
{
    ...
    connectionConvertBdaddr_t(&prim->connection.bd_addr, &req->bd_addr);
    prim->connection.service.protocol_id = connectionConvertProtocolId(req->protocol_id);
...
{
    if (cfm->context && cfm->connection.service.protocol_id == SEC_PROTOCOL_RFCOMM)
    {
        MAKE_CL_MESSAGE(CL_SM_REGISTER_OUTGOING_SERVICE_CFM);
        connectionConvertBdaddr(&message->bd_addr,&cfm->connection.bd_addr);
        message->security_channel = cfm->connection.service.channel;
        MessageSend((Task)cfm->context, CL_SM_REGISTER_OUTGOING_SERVICE_CFM, message);
    }
}
...
*/
void connectionHandleEncryptionKeyRefreshInd(const DM_HCI_REFRESH_ENCRYPTION_KEY_IND_T *ind)
{
	bdaddr addr;
	uint16 i = 0;
	uint16 max_sinks = 6;		
...
*/
void handleSetTrustLevelReq(const CL_INTERNAL_SM_SET_TRUST_LEVEL_REQ_T* req)
{
    connectionAuthSetTrustLevel(&req->bd_addr, req->trusted);
}
...
void handleAuthoriseRes(const CL_INTERNAL_SM_AUTHORISE_RES_T* res)
{
    ...
    connectionConvertBdaddr_t(&prim->cs.connection.bd_addr, &res->bd_addr);
    prim->cs.connection.service.protocol_id = connectionConvertProtocolId(res->protocol_id);
...
void handleAddAuthDeviceReq(connectionReadInfoState *infoState, connectionSmState *smState, const CL_INTERNAL_SM_ADD_AUTH_DEVICE_REQ_T *req)
{
    ...
    else
    {
        ...
        if (!connectionAuthAddDevice(&req->bd_addr, req->link_key_type, req->link_key, req->trusted, req->bonded))
        {
            MAKE_CL_MESSAGE(CL_SM_ADD_AUTH_DEVICE_CFM);
...
void handleGetAuthDeviceReq(connectionSmState *smState, const CL_INTERNAL_SM_GET_AUTH_DEVICE_REQ_T *req)
{
	...
	else
	{
		...
		if (connectionAuthGetDevice(&req->bd_addr, &link_key_type, &linkkey[0], &trusted))
		{
            ...
            memmove(&message->link_key[0], &linkkey[0], SIZE_LINK_KEY);
		}
		else
		{
			message->status = fail;
...
*/
void connectionDmHandleAclDetach(const CL_INTERNAL_DM_ACL_FORCE_DETACH_REQ_T *req)
{
