...
#define for_all_services(service, idx)      for(service = &theHfp->services[idx=0]; idx < theHfp->num_supported_profiles; service = &theHfp->services[++idx])
...
hfp_service_data* hfpGetServiceFromChannel(uint8 channel)
{
    ...
    for_all_services(service, index)
