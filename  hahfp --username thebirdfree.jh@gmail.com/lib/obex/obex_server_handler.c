...
void obexSendResponse( Obex session, ObexResponse response )
{
    ...
    if( response < obex_continue ) response = obex_unknown_error;

    /* Assuming the respose for PUT. It can change later */
    ...
    if( response != obex_continue ) SET_OBEX_CONNECTED( session );
