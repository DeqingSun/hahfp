...
.endif
   ...
   .VAR/DM1 mute_cntrl_l_dm1[$music_example.MUTE_CONTROL.STRUC_SIZE] =
      0,                                     // Input/Output pointer
      0,                                     // Buffer length
      $music_example.NUM_SAMPLES_PER_FRAME,  // Number of samples to process
      1;                                     // Mute flag (always mute)

   .VAR/DM1 mute_cntrl_r_dm1[$music_example.MUTE_CONTROL.STRUC_SIZE] =
      0,                                     // Input/Output pointer
      0,                                     // Buffer length
      $music_example.NUM_SAMPLES_PER_FRAME,  // Number of samples to process
      1;                                     // Mute flag (always mute)
...
.endif
    &mute_cntrl_l_dm1 + $music_example.MUTE_CONTROL.OFFSET_INPUT_PTR,
    &mute_cntrl_l_dm1 + $music_example.MUTE_CONTROL.OFFSET_INPUT_LEN,
...
.endif
    &mute_cntrl_r_dm1 + $music_example.MUTE_CONTROL.OFFSET_INPUT_PTR,
    &mute_cntrl_r_dm1 + $music_example.MUTE_CONTROL.OFFSET_INPUT_LEN,
...
.endif
   ...
   .VAR StandBy_proc_funcs[] =
    // Function                                 r7                   r8
       ...
       $M.audio_proc.peak_monitor.Process.func, &pcmin_l_pk_dtct,    0,
       $M.audio_proc.peak_monitor.Process.func, &pcmin_r_pk_dtct,    0,
       // Always muted so no $stream_copy is performed
       $mute_control.Process.func,              &mute_cntrl_l_dm1,   0,
       $mute_control.Process.func,              &mute_cntrl_r_dm1,   0,
       $M.audio_proc.peak_monitor.Process.func, &dac_l_pk_dtct,      0,
       $M.audio_proc.peak_monitor.Process.func, &dac_r_pk_dtct,      0,
       $frame.update_streams,                   &stream_setup_table, &$cbuffer_strucs,
       0;
