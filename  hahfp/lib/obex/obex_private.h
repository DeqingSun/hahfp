...
#define     OBEX_MAX_PACKET_SIZE    (948)
...
#define IsObexServer( session ) (session->role == obex_server)
#define IsObexClient( session ) (session->role == obex_client)
...
struct __obex
{
    ...
    uint8        rfcChannel;     /* The associated rfcomm Channel */
