...
/*! @brief filters for messages listing. Currently it filters 
    ...
    parameter mask. One or more values can be bit ORed to to use the filter
...
/*! @brief The message generated for MapcMnsStart() 
...
/*! @brief The message generated for MapcMnsShutdown()
...
/*! @brief Message indication for incoming connect request from the MSE
...
/*! @brief The message generated for MapcMnsConnectResponse()
...
/*! @brief The message generated for MapcMasConnectRequest()
...
/*! @brief  The message generated for MapcMasSetNotificationRequest()
...
/*! @brief The message generated for MapcMasSetFolderRequest()
...
/*! @brief The message generated for MapcMasGetFolderListingRequest().
    ...
    listing, if the status is mapc_continue.  
*/ 
typedef struct{
    Mas         masSession;     /*!< The session handle of the MAS session */ 
    MapcStatus  status;         /*!< Status of the operation */
...
/*! @brief The message generated for MapcMasGetMessagesListingRequest().
...
/*! @brief The message generated for MapcMasGetMessageRequest().

    The confirmation Message on receiving the message from the MSE.
    The application must either call MapcMasGetComplete() to terminate the
    operation or call MapcMasGetContinue() to get the rest of the message,
    if the status is mapc_continue 
*/ 
typedef struct{
    Mas         masSession;     /*!< The session handle of the MAS session */
    MapcStatus  status;         /*!< Status of the operation */
...
/*! @brief The message generated for MapcMasPutMessage().
    ...
    status is mapc_continue ). The final success confirmation message will
...
/*! @brief  The message generated for MapcMasUpdateInbox()
...
/*! @brief  The message generated for MapcMasSetMessageStatusRequest()
...
/*!
    ...
    Notification by calling MapMnsSetRegisterNotification(). The
    library returns a message MAPC_MNS_START_CFM with the status of the
    MNS service. The returned status are mapc_success on successfully
...
/*!
    ...
    @param masSession  The session handle of the MNS session to be deleted.

    The application must call this API on receiving MAPC_MNS_DISCONNECT_IND
...
/*!
    ...
    @param bool  TRUE to accept the connection.

    @param connectID  The Connection ID (Sink) provided in the 
    MAPC_MNS_CONNECT_IND message.

    The application must call this API on receiving the message
    MAPC_MNS_CONNECT_IND from the library for incoming connections. The 
    library returns one or more MAPC_MNS_CONNECT_CFM as a result of this
    API Call. After establishing a session, the library notifies 
    MAPC_MNS_CONNECT_CFM to the application with  mapc_progressing.
    After a successful MNS Connection, it notifies MAPC_MNS_CONNECT_CFM
    with mapc_success.
...
/*!
    ...
    The application will receive  CL_SDP_SERVICE_SEARCH_ATTRIBUTE_CFM 
    with the attributes.  The application can use sdp_parse library to 
    extract the values. 
...
/*!
    ...
    @param bdaddr  The Bluetooth device address of the remote MSE device.

    @param masChannel  The MAS RFCOMM Channel. The application can extract
    a valid remote MAS channel from the result of MapcMasSdpAttrSearch().
    ...
    The library returns one or more MAPC_MAS_CONNECT_CFM as a result of this
    API Call. After establishing a session, the library notifies 
    MAPC_MAS_CONNECT_CFM to  the application with  mapc_progressing for 
    each session in progress. After a successful MAS Connection, it notifies
    MAPC_MAS_CONNECT_CFM with mapc_success.
*/
...
/*!
    ...
    The library returns MAPC_MAS_DISCONNECT_IND message on disconnecting the 
...
/*!
    ...
    The application may call this API after receiving the 
    MAPC_MAS_DISCONNECT_IND  to confirm the disconnection and 
    release of session resources.
...
/*!
    ...
    The library returns a message MAPC_MAS_SET_NOTIFICATION_CFM after 
...
/*!
    ...
    The application must call this API on receiving the message 
    MAPC_MNS_SEND_EVENT_IND from the Mapc library. The application shall not 
...
/*!
    ...
    @param len  The length on the name string.
     ...
     The API function to navigate the folders of the MSE. The library returns
     MAPC_MAS_SET_FOLDER_CFM message after processing this API.
...
/*!
    ...
    current folder of the MSE.The library returns the message 
    MAPC_MAS_GET_FOLDER_LISTING_CFM with the result after completing 
    this operation. The maxListCount value must not be zero.
*/
...
/*!
    ...
    current folder of the MSE.The library returns the message 
    MAPC_MAS_GET_MESSAGES_LISTING_CFM with the result after completing 
    this operation. The maxListCount value must not be zero.
*/
...
/*!
    ...
    of the MSE. The library returns the message MAPC_MAS_GET_MESSAGE_CFM
    with the result.  The size of the handle must be 16 character length, 
    otherwise the library panic.
*/
...
/*!
    ...
    new messages from the network. The library returns 
    MAPC_MAS_UPDATE_INBOX_CFM as a result of this call.
*/
...
/*!
    ...
    or unread. The library returns MAPC_MAS_SET_MESSAGE_STAUS_CFM as a 
    result of this call.
*/
...
/*!
    ...
    The library returns MAPC_MAS_PUT_MESSAGE_CFM on completion of this 
    request. The application must call MapcMasPutContinue() if the  
    return status is mapc_continue.
