...
#define  GOEP_SESSION_CFM_T     CL_RFCOMM_CLIENT_CONNECT_CFM_T 
#define  GOEP_DISCONNECT_CFM_T  CL_RFCOMM_DISCONNECT_CFM_T
...
static void handleRfcConnectCfm(goepState *state, GOEP_SESSION_CFM_T *msg);
static void handleRfcConnectInd(goepState *state, CL_RFCOMM_CONNECT_IND_T *msg);
static void handleRfcDisconnectInd(goepState *state, GOEP_DISCONNECT_CFM_T *msg);
...
void goepHandler(Task task, MessageId id, Message message)
{
    ...
    case CL_RFCOMM_SERVER_CONNECT_CFM:
		handleRfcConnectCfm(state, (GOEP_SESSION_CFM_T*)message);
		break;
    ...
    case CL_RFCOMM_DISCONNECT_CFM:
		handleRfcDisconnectInd(state, (GOEP_DISCONNECT_CFM_T*)message);
		break;
...
static void handleRfcConnectCfm(goepState *state, GOEP_SESSION_CFM_T *msg)
{
    if( msg->status == rfcomm_connect_pending ) 
    {
	    GOEP_DEBUG(("CL_RFCOMM_CONNECT_CFM, Conneection is pending\n"));
	
        /* Connection is still Pending */
        return;
    }
    else if (msg->status == rfcomm_connect_success ||
             msg->status == rfcomm_connect_channel_already_open)
	{
		state->sink=msg->sink;
        
		GOEP_DEBUG(("CL_RFCOMM_CONNECT_CFM, Conneection success\n"));
...
static void handleRfcConnectInd(goepState *state, CL_RFCOMM_CONNECT_IND_T *msg)
{
	...
	if (state->role == goep_Server)
	{
		if ((state->state == goep_initialised) || (state->state == goep_connecting))
		{
			...
			ConnectionRfcommConnectResponse(&state->task, TRUE, msg->sink, msg->server_channel, NULL);
		}
		else
		{
			ConnectionRfcommConnectResponse(&state->task, FALSE, msg->sink, msg->server_channel, NULL);
		}
	}
	else
...
static void handleRfcDisconnectInd(goepState *state, GOEP_DISCONNECT_CFM_T *msg)
{
    GOEP_DEBUG(("CL_RFCOMM_DISCONNECT_IND or CL_RFCOMM_DISCONNECT_CFM\n") ); 
