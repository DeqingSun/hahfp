                                ...
                                uint16          packet_size)
{
    ...
    case AVCTP0_PACKET_TYPE_START:
        ...
        /* Fragmentation is accepted only for Pass through Vendor 
           ...
           AVC Commands. 
           Minimum length is AVCTP Start Header of 4 + 
                               Venor ID / Vendor specific header size of 10 
        */
    ...
    case AVCTP0_PACKET_TYPE_SINGLE: /* Fall through */
        ...
        /* Check if we have received any fragmented pkts before
           if av_msg is a pointer to source, it should not come here */
        if(avrcp->av_msg_len)
        {
            /* Silently drop it */
            free(avrcp->av_msg);
        ...
        if ((ptr[pid_offset] != AVCTP1_PROFILE_AVRCP_HIGH) ||
            (ptr[pid_offset+1] != AVCTP2_PROFILE_AVRCP_REMOTECONTROL))
        {
        #ifndef AVRCP_CT_ONLY_LIB /* Disable TG for CT only lib */
            if(cr_type == AVCTP0_CR_COMMAND)
            {
                ...
                avrcpSendAvcResponse(avrcp,0,avctp_response_bad_profile, 
                0, NULL);
            }
            else
        #endif /* AVRCP_CT_ONLY_LIB */
        ...
        {
            /* Allocate memory only for the header of the message */
            avrcp->av_msg= (uint8*)PanicUnlessMalloc( 
                                        AVRCP_AVC_START_HEADER_SIZE );
            avrcp->av_msg_len=packet_size;
            /* Split the data in sink and av_msg */
            memmove(avrcp->av_msg, ptr, AVRCP_AVC_START_HEADER_SIZE);
            avrcp->av_msg[AVCTP_NUM_PKT_OFFSET]--;
            avrcp->fragment = avrcp_packet_type_start;
            SourceDrop(StreamSourceFromSink(avrcp->sink), 
                                            AVRCP_AVC_START_HEADER_SIZE);
        }
        result = TRUE;
        break;
    ...
    case AVCTP0_PACKET_TYPE_END:  
      ...
      /* Check the transaction ID */
       ...
       {
            avrcpAvctpBadPacketHandler(avrcp, (const uint8*)avrcp->av_msg, 
                                       packet_size, cr_type);
            break;
       }
        ...
        /* Reassemble the Continuation Packet :
           avrcp->av_msg will have 10 bytes header, chop the 3 bytes 
           AVCTP header from sink and keep the rest in sink */
