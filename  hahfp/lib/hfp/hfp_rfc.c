...
void hfpHandleRfcommConnectInd(const CL_RFCOMM_CONNECT_IND_T *ind)
{
    ...
    if(service && !service->busy)
    {
        if(!link)
        {
            /* Check we have the resources to continue */
            link = hfpGetIdleLink();
            if(link)
            {
                /* Accept the request */
        ...
        else if(link->ag_link_loss_state == hfp_link_loss_recovery)
        {
            /* Try and abort the next retry if the phone is reconnecting */
            ...
            {
                /* We managed to stop the retry, accept incoming */
                message->accepted = TRUE;
            }
        }
    }
    ...
    ConnectionRfcommConnectResponse(&theHfp->task, message->accepted, ind->sink, ind->server_channel, NULL);
...
static void hfpHandleRfcommConnectCfm(hfp_link_data* link, Sink sink, rfcomm_connect_status status)
{
    ...
    if(link)
    {
        ...
        hfpSendSlcConnectCfmToApp(link, cfm_status);
    }
}
...
static void hfpHandleRfcommDisconnect(Sink sink, rfcomm_disconnect_status status)
{
    ...
    if (link->ag_slc_state < hfp_slc_complete)
    {
        ...
        hfpSendSlcConnectCfmToApp(link, hfp_connect_slc_failed);
    }
    else
    {
        if (link->audio_sink)
