// *****************************************************************************
// Copyright (C) Cambridge Silicon Radio plc 2003-2009        http://www.csr.com
// Part of Stereo-Headset-SDK
//
// $Revision$  $Date$
// *****************************************************************************
...
.VAR $tmr_period_codec_copy=TMR_PERIOD_CODEC_COPY;
...
.ifndef HARDWARE_RATE_MATCH
      // disable software rate matching
      r0 = &$audio_out_dither_and_shift_op_left;
      M[$audio_out_dc_remove_op_right + 0] = r0;
      r0 = &$audio_out_dither_and_shift_op_mono;
      M[$audio_out_dc_remove_op_mono + 0] = r0;
.endif
      jump $pop_rLink_and_rts;

   is_remote_stream:

   // If rate matching is disabled don't update rate (and disable S/W rate matching)
   null = M[&$rate_match_disable];
   if NZ jump rate_match_disabled;
