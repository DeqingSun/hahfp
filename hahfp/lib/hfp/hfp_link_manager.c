    ...
    if(sink != NULL)
    {
        for_all_links(link, index)
        {
            ...
            else if(link->ag_slc_state < hfp_slc_outgoing)
            {
                ...
                if(SinkGetBdAddr(sink, &check_addr) && BdaddrIsSame(&check_addr, &link->identifier.bd_addr))
                    return link;
            }
            else
            {
...
hfp_link_data* hfpGetLinkFromAudioSink(Sink sink)
{
    ...
    for_all_links(link, index)
    {
        if(link->audio_state < hfp_audio_connected)
        {
            ...
            if(SinkGetBdAddr(sink, &bd_addr) && hfpGetLinkBdaddr(link, &link_addr) && BdaddrIsSame(&link_addr, &bd_addr))
                return link;
        }
        else
        {
...
hfp_link_data* hfpGetLinkFromPriority(hfp_link_priority priority)
{
    ...
    if(!theHfp)
        HFP_ASSERT_FAIL(("HfpInit has not been called!\n"));
    
    if(theHfp->initialised && priority != hfp_invalid_link)
        for_all_links(link, index)
