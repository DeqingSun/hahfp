...
static remote_device* findDeviceFromSink (Sink sink)
{
    ...
    if (sink != NULL)
    {
        ...
        if ( SinkIsValid(sink) && SinkGetBdAddr(sink, &addr) )
        {
            for (i=0; i<A2DP_MAX_REMOTE_DEVICES; i++)
            {
                if ( BdaddrIsSame( &a2dp->remote_conn[i].bd_addr, &addr ) )
                {
                    PRINT(("[%04X %02X %06lX] sink=%X id=%u ",addr.nap,addr.uap,addr.lap,(uint16)sink,i));
...
static bool completeSignalling (signalling_channel *signalling, Sink sink, uint16 mtu)
{
...
static bool completeMedia (media_channel *media, Sink sink, uint16 mtu)
{
    PRINT((" completeMedia media=%X sink=%X mtu=%u\n",(uint16)media, (uint16)sink, mtu));
    ...
    if (device != NULL)
    {
        for (i=0; i<A2DP_MAX_MEDIA_CHANNELS; i++)
        {
            ...
            if ( ((media->status.connection_state == avdtp_connection_paging) || 
                  (media->status.connection_state == avdtp_connection_paged) ||
                  (media->status.connection_state == avdtp_connection_crossover)) && 
                 ((media->connection.setup.outbound_cid == connection_id) || (media->connection.setup.inbound_cid == connection_id)) )
            {
                return media;
            }
        }
...
static bool isSignallingDisconnectPending (remote_device *device)
{
    if (device != NULL)
    {
        if (device->signal_conn.status.connection_state == avdtp_connection_disconnect_pending)
        {
            return TRUE;
        }
    }
...
/*****************************************************************************/
void a2dpHandleL2capSignallingConnectReq(const A2DP_INTERNAL_SIGNALLING_CONNECT_REQ_T *req)
{
    ...
    {   /* Outgoing request to a new device */
        ...
        {   /* Able to support a new connection */
            ...
            ConnectionL2capConnectRequest(&a2dp->task, &req->addr, AVDTP_PSM, AVDTP_PSM, sizeof(a2dp_conftab), (uint16*)a2dp_conftab);
        }
        else
...
/*****************************************************************************/
void a2dpL2capMediaConnectReq(remote_device *device)
{
    /* We are connecting/have connected to this device */
    signalling_channel *signalling = &device->signal_conn;
    
    PRINT(("a2dpL2capMediaConnectReq signalling state=%u",signalling->status.connection_state));
    switch (signalling->status.connection_state)
    {
    ...
    case avdtp_connection_connected:
        ...
        {   /* No media channel currently being setup, ok to request one */
            ...
            {   /* Able to initiate a media connection */
                ConnectionL2capConnectRequest(&a2dp->task, &device->bd_addr, AVDTP_PSM, AVDTP_PSM, sizeof(a2dp_conftab), (uint16*)a2dp_conftab);
            }
            else
    ...
    if(ind->psm == AVDTP_PSM)
    {
        ...
        {   /* We are connecting/have connected to this device */
            ...
            case avdtp_connection_connected:
                ...
                if ( (media=initiateMedia(device, ind->connection_id, ind->identifier))!=NULL )
                ...
                {   /* Unable to accept any more media connections */
                    ConnectionL2capConnectResponse(&a2dp->task, FALSE, ind->psm, ind->connection_id, ind->identifier, sizeof(a2dp_conftab), (uint16*)a2dp_conftab);
                }
                break;
...
/*****************************************************************************/
void a2dpHandleL2capConnectRes(const A2DP_INTERNAL_SIGNALLING_CONNECT_RES_T *res)
{
    signalling_channel *signalling = &((remote_device *)PanicNull(res->device))->signal_conn;
    
    PRINT(("a2dpHandleL2capConnectRes device=%X",(uint16)res->device));    
...
void a2dpHandleL2capConnectCfm(const CL_L2CAP_CONNECT_CFM_T *cfm)
{
    ...
    if (device != NULL)
    {
        ...
        case avdtp_connection_paged:
            ...
            {
                completeSignalling(signalling, cfm->sink, cfm->mtu_remote);
            ...
            else
            {
                ...
                {   /* A singular connection setup being managed */
                    ...
                    {   /* Trying to re-establish a signalling channel after a link loss */
                        if ( !device->linkloss_timer_expired )
        ...
        case avdtp_connection_connected:
            /* Signalling channel connected, cfm message likely to be for a media channel */
            media = findMediaFromConnectionId( device, cfm->connection_id );
            /* If we couldn't find this connection ID it may be an outgoing connection */
            if( media == NULL ) media = findMediaFromConnectionId( device, AVDTP_OUTGOING_CONNECTION_ID );
            ...
            if ( media != NULL )
            {
                ...
                case avdtp_connection_paged:
                    if (cfm->status == l2cap_connect_success)
                    {
                        completeMedia(media, cfm->sink, cfm->mtu_remote);
            ...
            {   /* Received a confirmation message with an unrecognised connection id - should be due to a signalling crossover condition */
                PRINT((" unrecognised cid\n"));
                PanicFalse(cfm->status != l2cap_connect_success);  /* TODO: We have a situation here - two signalling channels to the same device */
            }
            break;
...
/****************************************************************************/
void a2dpHandleL2capSignallingDisconnectReq(const A2DP_INTERNAL_SIGNALLING_DISCONNECT_REQ_T *req)
{
    /* We should already know about the device sending us a message */
    ...
    if ( device )
    {
        ...
        case avdtp_connection_connected:
            ...
            {   /* Client is pulling the signalling channel while not in an idle state - we have no option but to abort */
                a2dpStreamAbort(device);
...
/****************************************************************************/
void a2dpL2capMediaDisconnectReq(Sink sink)
{
    /* We should already know about the device sending us a message */
    remote_device *device = findDeviceFromSink(sink);
    
    PRINT(("a2dpL2capMediaDisconnectReq device=%X sink=%X", (uint16)device, (uint16)sink));  
...
void a2dpHandleL2capDisconnect(uint16 cid, Sink sink, l2cap_disconnect_status status)
{
    ...
    {   /* Device is recognised by A2DP lib */
        ...
        {   /* A signalling channel disconnected */
            ...
            case avdtp_connection_connected:
            case avdtp_connection_disconnect_pending:
                /* This will be a disconnect indication message.  Respond as we are not in a disconnect crossover situation */
                ConnectionL2capDisconnectResponse(cid, sink);
                /* Drop through to avdtp_connection_disconnecting case */
        ...
        {   /* A media channel disconnected */
            ...
            case avdtp_connection_disconnecting:
                ...
                if ( !isMediaConnected( device ) )
                {
                    ...
                    if ( isSignallingDisconnectPending(device) )
                    {   /* Signalling channel disconnect is pending - do it now */
                        signalling->status.connection_state = avdtp_connection_disconnecting;
                        ConnectionL2capDisconnectRequest(&a2dp->task, signalling->connection.active.sink);
                    }
                    else if ( !isSignallingConnected( device ) )
