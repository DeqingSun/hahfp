...
DESCRIPTION
    ...
    Security Manager
*/
static void register_device(const TrustedDeviceRecordType* p_dev_record)
{	
...
*/
static uint16 update_trusted_device_list (const uint16 position, 
										  ...
										  uint16 bonded)
{
    ...
    if(link_key_type != cl_sm_link_key_debug)
	memmove(record.link_key, link_key, SIZE_LINK_KEY);
    else
        memset(record.link_key, 0, SIZE_LINK_KEY);
...
*/
uint16 connectionInitTrustedDeviceList()
{
...
FUNCTION
	...
	returned.
*/
uint16 connectionAuthAddDevice(const bdaddr* peer_bd_addr, cl_sm_link_key_type link_key_type, const uint8* link_key, uint16 trusted, uint16 bonded)
{
	...
	return update_trusted_device_list(position, peer_bd_addr, link_key_type, link_key, trusted, bonded);
}
...
*/
uint16 connectionAuthSetTrustLevel(const bdaddr* peer_bd_addr, uint16 trusted)
{
    /* Holds the position of a device in the trusted device list (TDL) */
	uint16  rec = 0;
    TrustedDeviceRecordType record;
...
*/
void connectionAuthSendLinkKeyOld(const bdaddr* peer_bd_addr)
{
	uint16						rec;
    TrustedDeviceRecordType     record;
	
    MAKE_PRIM_T(DM_SM_LINK_KEY_REQUEST_RSP);
...
*/
bool connectionAuthSendLinkKey(const bdaddr* peer_bd_addr, bool authenticated)
{
	uint16						rec;
    TrustedDeviceRecordType     record;
	uint8*						link_key;
	...
	if(rec)
	{
        ...
        /* Success if authenticated key was requested and we have one, or any key was requested and we have a key that isn't type none/debug */
        if((authenticated && record.link_key_type == cl_sm_link_key_authenticated) || 
           (!authenticated && record.link_key_type != cl_sm_link_key_none && record.link_key_type != cl_sm_link_key_debug))
			{
