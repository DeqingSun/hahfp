...
/*!
    ...
    Set this value greater than RTX+ERTX Timeout. To avoid any race with Bluestack Timeout Indication, set this value greater than 120s
    Otherwise Connection Library always Time Out exactly with RTX Timeout and disconnects the connection.
    This will trigger only when Bluestack is not responding for L2CAP Connection.
*/
#define    DEFAULT_L2CAP_CONNECTION_TIMEOUT    (125)



/*!  Default L2CAP Local MTU Maximum
...
*/
#define RFCOMM_DEFAULT_MODEM_SIGNAL (RFC_MS_DV_BIT|RFC_MS_RTS_BIT|RFC_MS_RTC_BIT)
...
/*!
    ...
    proceeds without it.
*/
#define RFCOMM_DEFAULT_MSC_TIMEOUT  250
...
/*!  RFCOMM Port Negotiation Baud Rate values */
...
/*!  RFCOMM Port Negotiation Data bit values */
...
/*!  RFCOMM Port Negotiation Flow Control bit masks */
...
/*!  Bluestack Key Value Pair macros
 ...
 */
 #define BKV_UINT16R(most_preferred,least_preferred)    BKV_UINT16V(least_preferred), BKV_UINT16V(most_preferred)
/*!
...
*/
#define BKV_UINT32R(most_preferred,least_preferred)    BKV_UINT32V(least_preferred),BKV_UINT32V(most_preferred)
/*!
    @brief Macro to combine Flow and Error Control Mode and the Mode fallback 
    mask into a single uint16 value
*/
#define BKV_16_FLOW_MODE( mode, fallback_mask)  ((((mode)<<8)&0xFF00)|(fallback_mask))


/*!  Bluestack Keys for the L2CAP Configuration Table (conftab)
...
/*!  L2CAP Flow & error control modes
...
/*! 
   @brief L2CAP flush timout (32-bit) - Infinite Flush Timeout 
*/ 
...
*/
#define L2CAP_PSM_REG_FLAG_ALLOW_RX_UCD 0x0001 /*!< Allow Unicast Connectionless Data Reception.*/
#define L2CAP_PSM_REG_FLAG_ALLOW_RX_BCD 0x0002 /*!< Allow Broadcast Connectionless Data Reception.*/
...
typedef enum
{
    ...
    hci_version_3_0,            /*!< HCI Versuin 3.0. */
...
/*! 
    ...
    bitwise ORed to provide composite security mask. Common bitwise masks are
...
typedef enum
{
    ...
    */
    ...
    /*! mode 4 - Security Level 2. MITM not necessary, Encryption desired. */
    sec4_out_level_2               = (sec4_out_ssp | sec_out_authentication | sec_out_encryption),
    /*! mode 4 - security Level 3. MITM necessary, Encryption disired, user 
    interaction is acceptable. */
    sec4_out_level_3               = (sec4_out_ssp | sec4_out_mitm | sec_out_authentication | sec_out_encryption)
} dm_security_out;
...
typedef enum
{
    ...
    sdp_open_search_failed_page_timeout,        /*!< The open search failed due to page timeout.*/
    sdp_open_search_failed_rej_psm,             /*!< The SDP server rejected SDP PSM.*/
    sdp_open_search_failed_rej_security,        /*!< The SDP server rejected due to security.*/
    sdp_open_search_failed_rej_resources,       /*!< The SDP server rejected due to resources.*/
    sdp_open_search_failed_signal_timeout,      /*!< Connection signalling timed out.*/
...
typedef enum
{
    ...
    sdp_connection_error_page_timeout,      /*!< Connection error: page timeout.*/
    sdp_connection_error_rej_psm,           /*!< Connection error: rejected PSM.*/
    sdp_connection_error_rej_security,      /*!< Connection error: rejected security.*/
    sdp_connection_error_rej_resources,     /*!< Connection error: rejected resources.*/
    sdp_connection_error_signal_timeout,    /*!< Connection error: signal timeout.*/
...
{   
    ...
    uint8   break_signal;          
    /*! Modem Status timeout during connection in milli-seconds. If the 
...
typedef enum
{
    /*! Overrun Error - Recieved character overwrote an unread character. */
...
typedef struct
{
	...
	sync_link_type  link_type;      /*!< Specifies whether a SCO or eSCO packet type was obtained. */
    bdaddr           bd_addr;        /*!< The bluetooth address of the device. */
...
typedef struct
{
    /*! The sink of the stream associated with the connectionless data and remote device.*/
    Sink                    sink;
...
typedef struct
{
    /*! L2CAP link identifer which needs to be passed back in the Disconnect 
      response. */
    uint8                       identifier;
...
typedef struct
{
    ...
    uint8   server_channel;                
    /*! The sink associated witht the connection. */
    Sink    sink;
...
{
    Sink                    sink;       /*!< The sink.*/
    rfcomm_control_status   status;     /*!< The RFCOMM Control Request Status.*/
...
/*! 
    ...
    the range 0x03 to 0xFFFF. This corresponds to an inquiry timeout range of 3.84
    to 83884.8 seconds.
...
/*!
    ...
    The Page Scan Type confihuration parameter indicates whether page
    scanning will be done using non-interlaced scan or interlaced scan.
*/
...
/*! 
    ...
    the link key manually and retry.it is recommented to keep this Value TRUE to 
    maintain interoperobility with legacy devices. 
...
#define ConnectionSmRegisterOutgoingL2cap(bd_addr,channel,security_level) ConnectionSmRegisterOutgoingService(NULL,(bd_addr),protocol_l2cap,(channel),(security_level))
...
#define ConnectionSmRegisterOutgoingRfcomm(theAppTask,bd_addr,security_level) ConnectionSmRegisterOutgoingService((theAppTask),(bd_addr),protocol_rfcomm,0,(security_level))
...
/*!
    ...
    @param bd_addr The Bluetooth address of the device to drop ACL connection with
	
	@param reason The HCI Reason code (as defined in hci.h NOT connection lib.. for now)

    @param detach_all If TRUE then drop ALL ACL connections, the bd_addr is ignored.
*/
...
/*! 
    ...
    ConnectionSmPutAttribute() function).

    @param ps_base The base user persistent store key used to store attibute data.

	On return, all devices will have been deleted from the paired device list.  The
    attribute entries will also be deleted starting from the key defined by ps_base.
*/
...
/*! 
    ...
    A CL_SM_GET_INDEXED_ATTRIBUTE_CFM message, containing the requested attribute
    will be sent in response.
*/
...
/*! 
    ...
    @param search_pattern The pattern to search for.
*/
void ConnectionSdpServiceSearchRefRequest(Task theAppTask, const bdaddr *bd_addr, uint16 max_num_recs, uint16 size_search_pattern, const uint8 *search_pattern);
...
/*! 
    ...
    @param attribute_list The attributes list.
*/
void ConnectionSdpAttributeSearchRefRequest(Task theAppTask, const bdaddr *bd_addr, uint16 max_num_recs, uint32 service_hdl, uint16 size_attribute_list, const uint8 *attribute_list);
...
*/
void ConnectionSdpServiceSearchAttributeRefRequest(Task theAppTask, const bdaddr *addr, uint16 max_attributes, uint16 size_search_pattern, const uint8 *search_pattern, uint16 size_attribute_list, const uint8 *attribute_list);
...
/*! 
    ...
    @param identifer The signal identifier. This is the identifier field from 
    the CL_L2CAP_CONNECT_IND message that must be repsonded to. This field 
...
/*!
    ...
    connectionless data fromt he remote device.

    @param addr The Bluetooth address of the remote device that the L2CAP
    connectionless data is to be mapped to.

    @param local_psm The local PSM being connected. This PSM must have been
    registered with the Connection library before a connection can be
    initiated.

    @param remote_psm The remote PSM being connected to.
...
/*!
    ...
    as CL_L2CAP_UNMAP_CONNECTIONLESS_IND)for the the Source associated with 
...
/*!
    ...
    @param sink The Sink identify the stream for the connectionless data to a remote device.
...
/*!
    ...
    channel, indicated by a CL_RFCOMM_DISCONNECT_IND messsage.
...
/*!
    @brief This function is called to request Port paramater negotiation with
...
*/
void ConnectionRfcommPortNegRequest(Task theAppTask, Sink sink, bool request, port_par *port_params);
...
/*!
    ...
    port_params strucutre, including setting all the bits of the parameter_mask
    field to 1.
...
/*!
    ...
    @param theAppTask The client task.
    @param sink The sink.
    @param break_signal 0-3-seconds in 200ms increments e.g. range 0-15 where 1 = 200ms and 15 = 3-seconds
    @param modem_signal The modem signals.

	This function should only be called if the Rfcomm connection is active.
*/
void ConnectionRfcommControlSignalRequest(Task theAppTask, Sink sink, uint8 break_signal, uint8 modem_signal);
...
/*!
    @brief Configure the page timeout to be used when attempting a connection to a remote device.

    @param page_timeout New page timeout to use in connection attempts. This value is specified 
    in Bluetooth slots. Set to zero to reset the page timeout back to a default value.

    This function will set the default page timeout used for all subsequent connection attempts. 
    The page timeout is not reset back to a default value by the connection library. To reset 
    the page timeout back to a default value this function will need to be called again.

    This function does not return a message to the client task.

    Changing the default page timeout can seriously impair the ability of a device to create an
    ACL. Use of this function is not recommended unless absolutely necessary.
*/
