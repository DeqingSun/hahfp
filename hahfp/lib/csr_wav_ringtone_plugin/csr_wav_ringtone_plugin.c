...
static void handleInternalMessage ( Task task , MessageId id, Message message ) 	;
	...
	/*the plugin task*/
const TaskData csr_wav_ringtone_plugin = { message_handler };
	...
	switch (id)
	{
		case (AUDIO_PLUGIN_PLAY_TONE_MSG ):
		{
			...
			else
			{
				...
				CsrWavRingtonePluginPlayTone ( tone_message->tone, 
...
static void handleInternalMessage ( Task task , MessageId id, Message message ) 	
{
	switch (id)
	{
        case MESSAGE_STREAM_DISCONNECT: /*a tone has completed*/
        {
            ...
            CsrWavRingtonePluginToneComplete();     
        }    
		break ;
