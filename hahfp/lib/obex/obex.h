...
/*!
    ...
    Connection ID header is handled internally.
*/
typedef enum
{
...
/*! 
    @brief Indication message on a Incoming RFCOMM connection.
    ...
    The application must response to the OBEX_CONNECT_IND using 
...
typedef struct
{
    ...
    uint16  frameSize;      /**< The frame size.*/
...
/*!
    ...
    The application must respond with ObexAuthReqResponse() API  on 
...
/*!
    ...
    The application must respond with ObexAuthClgResponse() API on receiving
    this message.
*/
typedef struct
{
...
/*!
    @brief Confirmation message on establishing a RFCOMM Connection 
*/
typedef struct
{
    Obex        session;          /**< Obex Session Handle */
...
/*!
    @brief Response for the ObexPutRequest().
...
/*!
    @brief Response for the ObexGetRequest().
...
/*!
    @brief Response for the ObexSetPathRequest().
...
/*!
    @brief Response for the ObexDeleteRequest().
...
/*!
    ...
    ObexDeleteSession() after completing all operations 
...
/*!
    ...
    @connParams  parameters required to establish OBEX session. This must
    not be NULL.    

    @return The connection context task associated with this session.

    The application will receive a OBEX_CREATE_SESSION_CFM message followed
    by a OBEX_CONNECT_CFM message. The OBEX_CREATE_SESSION_CFM on success 
...
/*!
    ...
    @param nameLen The length of folder name to navigate.
    @param name Name of the folder to be navigated.

    The library returns OBEX_SET_PATH_CFM with the status. If the flag is set 
    to obex_new_folder or obex_folder_in, the application must provide a 
    valid folderName and folderLen.
...
/*!
	...
	A OBEX_DELETE_CFM message is received on completion.
*/
void ObexDeleteRequest(Obex sesssion, uint16 sizeName, const uint8* objName);    
...
/*!
    ...
    receiving OBEX_CONNECT_IND message. The application will receive a 
    OBEX_CREATE_SESSION_CFM message followed by a OBEX_CONNECT_CFM message
...
Task ObexConnectResponse( Sink					sink,
                          uint8                 rfcChannel,
                          bool                  accept,
                          const ObexConnParams* pConnParams);   
...
/*
    ...
    The application must call this API on receiving OBEX_PUT_IND message.
...
/*!
    ...
    The application must call this API on receiving OBEX_GET_IND message.
...
/*!
    ...
    The application must call this API on receiving OBEX_SET_PATH_IND 
    message.
*/
...
/*!
    ...
    The application must call this API on receiving the OBEX_AUTH_REQ_IND
    message. This message is generated as part of OBEX connection 
    establishment procedure if OBEX authetication is requested by the 
    Server.

    The library returns OBEX_AUTH_RSP_CFM with the request-digest string
...
/*!
    ...
    The application must call this API on receiving the OBEX_AUTH_CLG_IND
    message. This message is generated as part of the OBEX connection 
    establishment procedure when OBEX authetication is requested by the 
    Server.

    At the end of the connection procedure the library returns 
    OBEX_CONNECT_CFM with status.
*/
...
/*! 
    ...
    delete the Object by calling ObexObjDelete( ) 
...
/*!
    ...
    Add a four byte unsigned header to the OBEX object.
*/
...
/*!
    @brief  Filter the requested header from the Source and drop others.
...
/*!
    ...
    Extract a 4 byte header from the source. On failure it returns 
    0xFFFFFFFF.
*/
...
/*!
    ...
    OBEX returns the remote server channel if it is a client connection,
    otherwise it returns the local server channel.
*/
