...
.include "cbuffer_macro.h"
   ...
   .CONST $STREAM_LATENCY_VARIANCE              $TIMER_PERIOD_AUDIO_COPY_SAMPLES*4;   
   ...
   .CONST  $ADC_PORT             ($cbuffer.READ_PORT_MASK  | $cbuffer.FORCE_PCM_AUDIO + 0);  
   .CONST  $DAC_PORT             ($cbuffer.WRITE_PORT_MASK | $cbuffer.FORCE_PCM_AUDIO + 0);   
...
// INPUT stream
DeclareMonoAdcInputStreamWithSideTone($adc_in,                             /* Name */ \
                                      $ADC_PORT,                           /* Port */ \
...
DeclareScoOutputStream($sco_out,                                            /* Name */ \
                       ...
                       $BLOCK_SIZE * $BUFFER_SCALING,                       /* Buffer Size */ \
                       -8,                                                  /* Shift */       \
                       $TIMER_PERIOD_AUDIO_COPY_SAMPLES);                   /* Copy Limit */ 

DeclareMonoDacOutputStreamWithSideTone($dac_out,                            /* Name */ \
                       $DAC_PORT,                                           /* Port */ \
                       $BLOCK_SIZE * 2,                       /* Buffer Size */ \
                       -8,                                                  /* Shift */       \
...
.MODULE $sco_in;
    ...
    .VAR/DM sco_decode_obj[$sco_decoder.STRUC_SIZE] = 
...
// Auristream/ADPCM = ~500 words of PM RAM
.ENDMODULE;
   ...
   // tone buffer size: 
   //   16k len=tone_pts_per_ms + jitter + TONE_BUF_HIST
   //    8k len=frm_sz+1   
   .CONST $TONE_BUF_SZ           $BLOCK_SIZE+1;  
   // 16k tone buffer size: len=frm_sz+4 (resample op needs 3 out history)
   .CONST $TONE_UP_BUF_SZ        $BLOCK_SIZE+4; 
...
.MODULE $M.main;
   ...
   .VAR $profiler.main[$profiler.STRUC_SIZE];   
$main:
   // Map Flash Bank
   ...
   // Initialize audio config handler
